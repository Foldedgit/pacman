...
# update the path based on cost
        for x in s1:
            ...
            pqueue.push((fullPath, x[0]), cost)

        for x in s2:
            for y in pqueue.heap:
                if y[2][1] == x[0]:
                    excost = problem.getCostOfActions(y[2][0])

            cost = problem.getCostOfActions(path + [x[1]])

            if excost > cost:
                fullPath = path + [x[1]]
                pqueue.update((fullPath, x[0]), cost)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not pqueue.isEmpty():
        ...
        if s:
            ...
            for x in s:
                upcost = cost + x[2]
                pqueue.push((path + [x[1]], x[0], upcost), upcost + heuristic(x[0], problem))
